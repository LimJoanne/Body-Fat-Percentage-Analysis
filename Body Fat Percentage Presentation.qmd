...
```{r}
#log linear
lm3 <- lm(log(Pct.BF) ~Age + Abdomen, data = bodyfat)
summary(lm3)
```
...
lm_linear_log <- lm(Pct.BF ~ log(Age) + log(Abdomen), data = bodyfat)
...
```{r}
#log-log
lm_log_log <- lm(log(Pct.BF) ~ log(Age) + log(Abdomen), data = bodyfat)
