...
Density:
```{r}
formula_str <- paste("Pct.BF ~", "Density")
model <- lm(formula_str, data=bodyfat)
...
Abdomen:
```{r}
formula_str <- paste("Pct.BF ~", "Abdomen")
model <- lm(formula_str, data=bodyfat)
...
residuals <- residuals(step.back.aic)  # Replace step.back.aic with your final model
fitted_values <- fitted(step.back.aic)  # Replace step.back.aic with your final model
...
bodyfat %>%
  ggplot() +
  aes(sample = residuals(lm_log_log)) +  # Replace step.back.aic with your final model
  geom_qq(size = 2) +
  geom_qq_line() +
  labs(title = "Q-Q Plot of Residuals for Normality Check")
```
